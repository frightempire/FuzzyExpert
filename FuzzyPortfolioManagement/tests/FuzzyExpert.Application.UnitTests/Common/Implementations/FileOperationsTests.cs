...
namespace FuzzyExpert.Application.UnitTests.Common.Implementations
{
    ...
    {
        private readonly string _readFilePath = Path.Combine(TestContext.CurrentContext.TestDirectory, "Common\\TestFiles\\ReadTestFile.txt");
        private readonly string _emptyFilePath = Path.Combine(TestContext.CurrentContext.TestDirectory, "Common\\TestFiles\\EmptyFile.txt");
        private readonly string _writeFilePath = Path.Combine(TestContext.CurrentContext.TestDirectory, "Common\\TestFiles\\WriteTestFile.txt");
        ...
        public void ReadFileByLines_ReturnsCorrectListOfStrings()
        {
            ...
            // Assert
            Assert.IsTrue(TestHelper.ListsAreSequentiallyEqual(lines, expectedLines));
        }
